...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Detect without scoring or alerts")
    parser.add_argument("--lookback", type=int, default=None, help="Hours to look back")
    parser.add_argument("--detector", type=str, default="all", help="all, sec_form4, sec_8k, warn_act, linkedin")
    ...
    if errors:
        ...
        if not args.dry_run:
            logging.error("Fix config errors or use --dry-run")
            sys.exit(1)

    if args.run_once:
        detectors = args.detector.split(",") if args.detector != "all" else None
        pipeline = Pipeline(dry_run=args.dry_run)
        result = pipeline.run(lookback_hours=args.lookback, detectors=detectors)
        ...
        print(f"T1 (alert + LinkedIn): {result.t1_count}")
        print(f"T2 (lead list): {result.t2_count}")
